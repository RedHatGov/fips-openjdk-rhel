...
certutil -A -d $WORKDIR/nssdb -a -n rootca -i $WORKDIR/certs/ca.cert.pem -t CT,C,C -f $PASSFILE
certutil -A -d $WORKDIR/nssdb -a -n subrootca -i $WORKDIR/certs/intermediate.cert.pem -t CT,C,C -f $PASSFILE
